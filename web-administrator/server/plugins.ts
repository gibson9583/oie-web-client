...
function discover(pluginDirs: string[]): Array<{ dir: string; manifest: ServerPluginManifest }> {
    const plugins: Array<{ dir: string; manifest: ServerPluginManifest }> = [];
    const seen = new Set();
    ...
    for (const pluginDir of pluginDirs) {
        ...
        for (const entry of fs.readdirSync(pluginDir, { withFileTypes: true })) {
            ...
            try {
                ...
                console.error(`[plugins] ${entry.name}: failed to read plugin.json: ${(e as Error).message}`);
            }
        }
    }
    return plugins;
...
export function install(app: Express, config: any) {
    ...
    function mountServerEntry(dir: string, manifest: ServerPluginManifest): void {
        ...
        try {
            ...
            console.error(`[plugins] ${manifest.id}: server entry failed to load: ${(e as Error).message}`);
        }
    }

    // Plugin assets: resolve the owning directory per request so newly
    // installed plugins serve immediately.
    app.get('/plugins/:id/*splat', (req: Request, res: Response) => {
        ...
        res.sendFile(file, { root: found.dir, dotfiles: 'deny' }, (err) => {
            if (err && !res.headersSent) res.status((err as any).statusCode || 404).end();
        });
    });

    // The frontend bootstraps from this manifest — freshly scanned each time.
    app.get('/webadmin/plugins.json', (req: Request, res: Response) => {
