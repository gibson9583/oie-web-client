 ...
 */
export function resolveEngine(config: WebAdminConfig, req: { headers?: http.IncomingHttpHeaders } | null | undefined): EngineTarget | null {
    ...
    if (!engines) return { url: config.engine.url, verifyTls: !!config.engine.verifyTls };
 ...
 */
export function respondEngineUnknown(req: http.IncomingMessage, res: http.ServerResponse): void {
    req.resume();
...
export function isTrustedPeer(remoteAddress: string | null | undefined, trusted?: Set<string> | null): boolean {
...
export function resolveForwardedFor(remoteAddress: string | null | undefined, priorXff?: string | string[] | null, trusted?: Set<string> | null): string | string[] | undefined {
    const peer = String(remoteAddress || '').replace(/^::ffff:/, '');
...
export function createApiProxy(config: WebAdminConfig) {
    ...
    return function apiProxy(req: Request, res: Response): void {
        ...
        const upstream = transport.request({
            ...
            const brokenResponse = () => { if (!res.writableEnded) res.destroy(); };
            upstreamRes.on('error', brokenResponse);
            upstreamRes.once('aborted', brokenResponse);
            upstreamRes.once('close', () => { if (!upstreamRes.complete) brokenResponse(); });
        ...
        upstream.on('error', (err: any) => {
            ...
            const causes = err.errors ? err.errors.map((e: Error) => e.message) : [err.message];
...
export function engineRequest(engine: EngineTarget, { method, path: reqPath, headers, body, timeoutMs = 120000, deadlineMs = 0 }: { method: string; path: string; headers?: http.OutgoingHttpHeaders; body?: Buffer | null; timeoutMs?: number; deadlineMs?: number }): Promise<{ status: number; headers: http.IncomingHttpHeaders; body: Buffer }> {
    return new Promise((resolve, reject) => {
        ...
        const upstream = transport.request({
            ...
            res.on('data', (c: Buffer) => {
                size += c.length;
                if (size > MAX_RESPONSE) { res.destroy(); fail(new Error('engine response too large')); return; }
                chunks.push(c);
            });
        ...
        if (deadlineMs > 0) deadline = setTimeout(() => upstream.destroy(new Error('engine request exceeded its deadline')), deadlineMs);
