...
Configure discovery, client ID, token policy, JIT provisioning, account bindings, and RBAC mapping after login under **Settings → OIDC Authentication**. The tab and its API are protected by the extension permission `manageOIDC`. Saving writes the engine's `<appdata>/oidc.properties` atomically and reloads the authorization plugin; **Test connection** verifies discovery before rollout.
