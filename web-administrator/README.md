...
Configure discovery, client ID, token policy, JIT provisioning, account bindings, and RBAC mapping after login under **Settings → OIDC Authentication** (fields unlock once **Enable OIDC login** is ticked; Save/Refresh/Test connection live in the tab's task pane). The tab and its API are protected by the extension permission `manageOIDC` — holders of the RBAC admin role carry it implicitly, so grant it explicitly only to non-admin roles. Saving persists the policy to the engine database (the engine's native plugin-properties store; a pre-existing `<appdata>/oidc.properties` is imported once as bootstrap) and reloads the authorization plugin; **Test connection** verifies discovery before rollout.
