 ...
 * Administrator's filter and transformer panes. Edits the polymorphic element
 * list (rules/steps) of one connector, using the step/rule editors registered
 * through the platform (TransformerStepPlugin / FilterRulePlugin equivalent).
 *
 * Classic layout: steps/rules table on top, Step + Generated Script tabs
 * below, and a right-hand Reference / Message Templates / Variables panel.
 *
...
export function register(platform) {
    platform.registerView('/channels/:channelId/filter/:metaDataId',
        (ctx) => renderEditor(platform, ctx, 'filter'), { title: 'Filter' });
    platform.registerView('/channels/:channelId/transformer/:metaDataId',
        (ctx) => renderEditor(platform, ctx, 'transformer'), { title: 'Transformer' });
    platform.registerView('/channels/:channelId/response/:metaDataId',
        (ctx) => renderEditor(platform, ctx, 'response'), { title: 'Response Transformer' });
}

async function renderEditor(platform, { params }, kindName) {
    const kind = KINDS[kindName];
    const isFilter = kindName === 'filter';

    /* ---- load channel (prefer in-flight edits from the store) ---------------- */

    let channel = platform.store.getState('editingChannel');
    if (!channel || channel.id !== params.channelId) {
        try {
    ...
    if (!connector) {
        ...
        platform.router.navigate(`/channels/${params.channelId}/edit`);
        return { el: loading() };
    }
    ...
    async function saveChannel() {
        ...
        try {
            if (platform.store.getState('editingChannelNew')) {
