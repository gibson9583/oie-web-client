...
let current: any = null;      // at most one context menu is ever open
let seq = 0;
const listeners = new Set();
const emit = () => listeners.forEach((f: any) => f());
...
export function openRadixContextMenu({ x, y, items }: any) {
    const id = ++seq;
    ...
    const close = ({ restore = true } = {}) => {
        ...
        if (restore && opener && opener.isConnected && (opener as any).focus) (opener as any).focus();
    };
...
function Menu({ entry }: any) {
    ...
    return (
        <DropdownMenu.Root
            ...
            modal={false}
            onOpenChange={(open: any) => { if (!open) close(); }}>
            <DropdownMenu.Trigger asChild>
                <span aria-hidden="true"
                    style={{ position: 'fixed', left: x, top: y, width: 0, height: 0 }} />
            </DropdownMenu.Trigger>
            <DropdownMenu.Portal>
                <DropdownMenu.Content ref={(node: any) => { entry.node = node; }} className="ctx-surface"
