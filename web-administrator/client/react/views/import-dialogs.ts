...
export async function resolveImportName(
    ...
    async function checkName(n: any, candidateId: any) {
        if (!n) { await alertWarning(`${rules.noun} name cannot be empty.`); return false; }
        const problem = rules.validate ? rules.validate(String(n)) : null;
        if (problem) { await alertWarning(problem); return false; }
        if (nameClash(n, candidateId)) { await alertWarning(`${rules.noun} "${n}" already exists.`); return false; }
        return true;
    }

    if (!(await checkName(name, tempId))) {
        if (!(await optionYesNo(rules.title, `Would you like to overwrite the existing ${rules.noun.toLowerCase()}?  Choose 'No' to create a new ${rules.noun.toLowerCase()}.`))) {
            let newName = name;
            do {
