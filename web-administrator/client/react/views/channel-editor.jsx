...
function promptSaveChanges(channel) {
...
function openAttachmentPropsModal(ap, markDirty) {
...
function openDataTypesModal(channel, version, markDirty) {
...
async function openDependenciesModal(channel, version, markDirty) {
...
function openAdvancedQueueSettings(dcp, markDirty, onDone) {
    const queueEnabled = !!dcp.queueEnabled;
    const sendFirst = queueEnabled && !!dcp.sendFirst;
    const draft = {
...
function openDebugDeployModal(channel, save) {
    const options = [
...
async function ensureTags(tagState, channel) {
...
function applyTagsToChannel(tagState, channel, version) {
...
function TagsField({ tagState, channel, version, markDirty }) {
...
function ChannelPropertiesPanel({ channel, version, isNewRef, tagState, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
    const props = channel.properties = channel.properties || {};
    channel.exportData = channel.exportData || {};
                        ...
                        <div className="flex flex-wrap gap-y-1.5 gap-x-[18px] mt-0 mx-0 mb-1">
                            ...
                            <label className="check">
                                <input type="checkbox" checked={!!props.storeAttachments}
                                    onChange={(e) => { props.storeAttachments = e.target.checked; markDirty(); bump(); }} />
                                Store Attachments
                            </label>
                        </div>
...
function MessageStoragePanel({ channel, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
    ...
    const box = (def) => (
        <label className="check" key={def.key}>
            <input type="checkbox" checked={!!props[def.key]} disabled={!!disabled[def.key]}
                onChange={(e) => { props[def.key] = e.target.checked; markDirty(); bump(); }} />
            {def.label}
        </label>
    );
...
function PruningPanel({ channel, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
                ...
                <div className="flex flex-col gap-1 mt-1.5">
                    ...
                    <label className="check">
                        <input type="checkbox" checked={!!pruning.pruneErroredMessages} disabled={nothingPruned}
                            onChange={(e) => { pruning.pruneErroredMessages = e.target.checked; markDirty(); bump(); }} />
...
function MetaDataColumnsPanel({ channel, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
    const props = channel.properties;
...
function FragmentRow({ col, commit, onDelete }) {
    return (
        <>
            <input type="text" value={col.name ?? ''} onChange={(e) => { col.name = e.target.value; commit(); }} />
            <select value={col.type || 'STRING'} onChange={(e) => { col.type = e.target.value; commit(); }}>
                {META_COLUMN_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            <input type="text" value={col.mappingName ?? ''} onChange={(e) => { col.mappingName = e.target.value; commit(); }} />
            <button className="btn w-full justify-center self-stretch" title="Remove column" onClick={onDelete}>Delete</button>
        </>
    );
}

function SummaryTab({ channel, version, isNewRef, tagState, markDirty }) {
    return (
            ...
            <div className="panel" style={{ marginTop: 0 }}>
                ...
                <div className="panel-body">
                    <textarea rows={4} placeholder="Describe what this channel does…"
                        ...
                        onChange={(e) => { channel.description = e.target.value; markDirty(); }} />
                </div>
            </div>
        </div>
    );
...
function ConnectorTypeSelect({ connector, mode, engineTypes, version, markDirty, onChanged, width }) {
    const [, bump] = useReducer((x) => x + 1, 0);
    const [engineList, setEngineList] = useState([]);
    const names = transportNamesFor(mode, connector.transportName);
    useEffect(() => {
        let stale = false;
        engineTypes().then((types) => { if (!stale) setEngineList(types); });
        return () => { stale = true; };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
...
function ConnectorPanelHost({ connector, mode, channel, markDirty, panelRev }) {
...
function RawConnectorProps({ connector, markDirty }) {
...
function SourceSettings({ channel, scp, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
        ...
        <div className="form-grid">
            ...
            <div className="field">
                ...
                <input key={respondAfter ? 'q-off' : 'q-on'} type="number" min={0} disabled={respondAfter}
                    ...
                    onChange={(e) => { scp.queueBufferSize = Number(e.target.value) || 0; markDirty(); }} />
            </div>
            <div className="field">
                <label>Response</label>
                <select value={currentResp}
                    onChange={(e) => { scp.responseVariable = e.target.value; markDirty(); bump(); }}>
            ...
            <div className="field">
                ...
                <select value={scp.processBatch ? 'yes' : 'no'}
                    onChange={(e) => { scp.processBatch = e.target.value === 'yes'; markDirty(); bump(); }}>
            ...
            <div className="field">
                ...
                <select disabled={!scp.processBatch} value={scp.firstResponse ? 'first' : 'last'}
                    onChange={(e) => { scp.firstResponse = e.target.value === 'first'; markDirty(); bump(); }}>
            ...
            <div className="field">
                ...
                <input type="number" min={1} defaultValue={scp.processingThreads ?? 1}
                    onChange={(e) => { scp.processingThreads = Number(e.target.value) || 1; markDirty(); }} />
            </div>
        </div>
    );
}

function SourceTab({ channel, version, engineTypes, markDirty }) {
...
function insertIntoField(t, token) {
...
function mappingResolveEditorAt(node) {
...
function DestinationSettings({ dcp, markDirty }) {
    const [, bump] = useReducer((x) => x + 1, 0);
    ...
    const ynRadios = (name, checked, onChange) => (
...
function DestEditor({ dest, channel, version, engineTypes, markDirty, syncRows }) {
    const [panelRev, setPanelRev] = useState(0);
    if (!dest) {
                    ...
                    <div className="dest-type-row flex items-center gap-2 flex-wrap">
                        ...
                        <label className="check dest-wait-push">
                            <input type="checkbox" checked={dest.waitForPrevious !== false}
                                onChange={(e) => { dest.waitForPrevious = e.target.checked; markDirty(); syncRows(); }} />
...
function DestinationsTab({ channel, version, engineTypes, markDirty, actionsRef, destTasksRef, onTasksChange, gotoElements }) {
    // Selection lives in the ref (execution-time reads); the state twin only
    // drives re-renders.
    const [, setSelectedIdState] = useState(null);
    const selectedIdRef = useRef(null);
    const setSelectedId = (id) => { selectedIdRef.current = id; setSelectedIdState(id); };
            ...
            },
            {
                ...
                render: (d) => {
                    const input = h('input.grid-name', {
                        ...
                        onInput: (e) => { d.name = e.target.value; markDirty(); }
                    });
...
function ScriptsTab({ channel, markDirty }) {
...
function EditorBody({ params, query, onTasksChange, apiRef, returning }) {
    ...
    const [, bumpRev] = useReducer((x) => x + 1, 0);
    const [activeTab, setActiveTabState] = useState('Summary');
    const activeTabRef = useRef('Summary');
    const setActiveTab = (label) => { activeTabRef.current = label; setActiveTabState(label); onTasksChange(); };
    const destTasksRef = useRef(null);
    ...
    const markDirtyImplRef = useRef(null);
    ...
    const engineTypesPromiseRef = useRef(null);
    const engineTypes = useMemo(() => () => {
        if (!engineTypesPromiseRef.current) {
            engineTypesPromiseRef.current = api.extensions.connectors().then((raw) => {
                const types = [];
    ...
    useEffect(() => {
        store.setState('navGuard', (info) => guardImplRef.current(info));
    ...
    apiRef.current = {
        ...
        handlers: {
            ...
            destStepCount: (key) => (destTasksRef.current ? destTasksRef.current.stepCountOf(key) : 0),
            ...
            destMove: (delta) => destTasksRef.current && destTasksRef.current.move(delta),
            destEdit: (kind) => destTasksRef.current && destTasksRef.current.editElements(kind),
