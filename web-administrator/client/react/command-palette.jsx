...
function viewEntries() {
    return platform.navItems()
        .filter((item) => !item.task || checkTask(item.rbac || 'view', item.task))
        .map((item) => ({
...
function commandEntries() {
    return allCommands()
        .filter((c) => !c.task || checkTask(c.rbac || 'view', c.task))
        .map((c) => ({
...
function channelEntries(fallback) {
    const seen = new Set();
...
export function CommandPalette() {
    ...
    const results = useMemo(() => {
        const scope = SCOPES[query[0]];
        const needle = (scope ? query.slice(1) : query).trim();
        const pool = scope ? entries.filter((e) => e.kind === scope.kind) : entries;
        ...
        if (!needle) {
            ...
            return found.length ? found : pool.filter((e) => e.kind === 'view').slice(0, MAX_RESULTS);
        }
        return pool
            .map((e) => {
    ...
    const run = useCallback((entry) => {
        if (!entry) return;
        setOpen(false);
        const recent = [entry.id, ...(getPref('paletteRecent') || []).filter((id) => id !== entry.id)];
