...
function attachmentsTable(properties, onChange, disabled) {
    ...
    function paint() {
        clear(wrap);
        rows.forEach((row, i) => {
            wrap.appendChild(h('div', { class: 'flex gap-1.5 mb-1.5' }, textInput(row.name, { placeholder: 'Name', disabled, class: 'flex-1', onInput: (e) => { row.name = e.target.value; commit(); } }), textInput(row.content, { placeholder: 'Content', disabled, class: 'flex-[2]', onInput: (e) => { row.content = e.target.value; commit(); } }), textInput(row.mimeType, { placeholder: 'MIME type', disabled, class: 'flex-1', onInput: (e) => { row.mimeType = e.target.value; commit(); } }), h('button.icon-btn', { type: 'button', title: 'Remove', disabled, onClick: disabled ? null : () => { rows.splice(i, 1); commit(); paint(); } }, icon('x'))));
        });
