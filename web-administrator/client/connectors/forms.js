...
export function buildForm(host, properties, fields, onChange) {
    ...
    function paint() {
        ...
        const openSection = (title) => {
            ...
            root.appendChild(h('div.cform-section', title ? h('div.cform-section-title', title) : null, grid));
        };
...
function renderRow(grid, properties, f, onChange, repaint, displays) {
    ...
    switch (f.type) {
        ...
        case 'radio': {
            ...
            control = h('div.radio-group.inline-row', (typeof f.options === 'function' ? f.options(properties) : f.options || []).map(opt => {
                const o = typeof opt === 'object' ? opt : { value: opt, label: String(opt) };
                return h('label.check', h('input', { type: 'radio', name, checked: String(o.value) === String(value ?? ''), onChange: () => set(o.value) }), o.label);
            }));
            break;
        }
        case 'display': {
    ...
    const cell = h('div.cform-control', { title: f.tooltip || null }, control, f.append ? f.append(properties, { onChange, repaint: repaint || (() => { }) }) : null);
    if (wide)
