...
function loadDrivers(platform) {
    if (!driversPromise)
        driversPromise = platform.api.server.databaseDrivers();
...
function driverControlNode(properties, platform, onChange) {
    ...
    function refresh() {
        loadDrivers(platform).then((drivers) => {
            ...
            for (const d of drivers) {
                if (d && d.className)
                    options.push({ value: d.className, label: d.name || d.className });
            }
    ...
    const initial = select([{ value: properties.driver ?? DRIVER_DEFAULT, label: properties.driver ?? DRIVER_DEFAULT }], properties.driver ?? DRIVER_DEFAULT, { onChange: (e) => { properties.driver = e.target.value; onChange(); } });
