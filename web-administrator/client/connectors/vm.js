...
function loadChannelNames(platform) {
    if (!channelNamesPromise)
        channelNamesPromise = platform.api.channels.idsAndNames();
...
function channelControlNode(properties, platform, onChange) {
    ...
    function syncCombo() {
        ...
        if (text.trim() === '') {
            ...
            if (match)
                ...
                selection = MAP_VARIABLE_LABEL;
            else
                selection = NOT_FOUND_LABEL;
        }
        ...
        for (const opt of Array.from(combo.options)) {
            if (opt.value === MAP_VARIABLE_LABEL || opt.value === NOT_FOUND_LABEL)
                opt.remove();
        }
    ...
    loadChannelNames(platform).then((map) => {
        ...
        for (const [name] of channelList)
            combo.appendChild(h('option', { value: name }, name));
        syncCombo();
    }).catch(() => { });
    return wrap;
}
...
function mapVariablesTable(properties, onChange) {
    ...
    function uniqueName() {
        for (let i = 1; i <= rows.length + 1; i++) {
            ...
            if (!rows.some((v) => v.toLowerCase() === name.toLowerCase()))
                return name;
        }
