...
const dicomListener = {
  ...
  component({ properties, onChange }) {
    return /* @__PURE__ */ React.createElement(ConnectorForm, { properties, onChange, fields: [
      ...
      {
        key: "bigEndian",
        label: "Accept Explict VR Big Endian",
        type: "radio",
        options: YES_NO,
        refresh: true,
        disabled: transferSyntaxLocked,
        onSet: (p) => {
          if (asBool(p.bigEndian)) p.defts = false;
        }
      },
      {
        key: "defts",
        label: "Only Accept Default Transfer Syntax",
        type: "radio",
        options: YES_NO,
        refresh: true,
      ...
      {
        key: "nativeData",
        label: "Only Uncompressed Pixel Data",
        type: "radio",
        options: YES_NO,
        refresh: true,
        disabled: transferSyntaxLocked,
        onSet: (p) => {
          if (asBool(p.nativeData)) p.defts = false;
        }
      },
