...
import { ConnectorForm, connectorTestButton, portsInUseButton, listenerAddressField, asBool, YES_NO, defaultSourceProperties, defaultDestinationProperties, defaultListenerProperties, CHARSETS, requireFields } from './react-forms.js';
...
function staticResourcesTable(properties, onChange) {
    ...
    function paint() {
        ...
        table.appendChild(h('thead', h('tr', h('th', 'Context Path'), h('th', 'Resource Type'), h('th', 'Value'), h('th', 'Content Type'), h('th'))));
        const body = h('tbody');
        rows.forEach((row, i) => {
            ...
            h('td', textInput(row.contextPath, { class: 'w-full', onInput: (e) => { row.contextPath = e.target.value; }, onChange: (e) => { row.contextPath = e.target.value; commit(); } })), h('td', select(RESOURCE_TYPES, row.resourceType, { onChange: (e) => { row.resourceType = e.target.value; commit(); } })), h('td', textInput(row.value, { class: 'w-full', onInput: (e) => { row.value = e.target.value; }, onChange: (e) => { row.value = e.target.value; commit(); } })), h('td', textInput(row.contentType, { class: 'w-full', onInput: (e) => { row.contentType = e.target.value; }, onChange: (e) => { row.contentType = e.target.value; commit(); } })), h('td', h('button.icon-btn', { type: 'button', title: 'Delete', onClick: () => { rows.splice(i, 1); commit(); paint(); } }, icon('x')))));
        });
    ...
    component({ properties, onChange }) {
        return (React.createElement(ConnectorForm, { properties: properties, onChange: onChange, fields: [
                ...
                { key: 'responseContentType', label: 'Response Content Type', type: 'text', width: '220px' },
                {
                    ...
                    onSet: (p) => { if (asBool(p.responseDataTypeBinary))
                        p.charset = DEFAULT_CHARSET; },
                    options: [
    ...
    component({ properties, channel, onChange }) {
        ...
        return (React.createElement(ConnectorForm, { properties: properties, onChange: onChange, fields: [
                ...
                { key: 'proxyPort', label: 'Proxy Port', type: 'number', width: '90px', disabled: (p) => !asBool(p.useProxyServer) },
                {
                    ...
                    onSet: (p) => { if (String(p.method) !== 'post')
                        p.multipart = false; },
                    options: [
                ...
                { section: 'HTTP Authentication' },
                {
                    ...
                    onSet: (p) => { if (!asBool(p.useAuthentication)) {
                        p.username = '';
                        p.password = '';
                    } }
                },
                ...
                { key: 'headersVariable', label: 'Map Variable', type: 'text', width: '320px', placeholder: 'e.g. RESTHeaders', disabled: (p) => !asBool(p.useHeadersVariable) },
                {
                    ...
                    onSet: (p) => { if (httpFormUrlEncoded(p)) {
                        p.multipart = false;
                ...
                },
                {
                    ...
                    onSet: (p) => { if (asBool(p.dataTypeBinary))
                        p.charset = DEFAULT_CHARSET; },
                    options: [
                        { value: true, label: 'Binary' },
                        { value: false, label: 'Text' }
