  ...
  function paint() {
    ...
    rows.forEach((row, i) => {
      body.appendChild(h(
        ...
        h("td", select(RESOURCE_TYPES, row.resourceType, { onChange: (e) => {
        ...
        h("td", h("button.icon-btn", { type: "button", title: "Delete", onClick: () => {
          rows.splice(i, 1);
          commit();
          paint();
        } }, icon("x")))
  ...
  component({ properties, onChange }) {
    return /* @__PURE__ */ React.createElement(ConnectorForm, { properties, onChange, fields: [
      ...
      { key: "responseContentType", label: "Response Content Type", type: "text", width: "220px" },
      {
        ...
        key: "responseDataTypeBinary",
        label: "Response Data Type",
        type: "radio",
        refresh: true,
        onSet: (p) => {
          if (asBool(p.responseDataTypeBinary)) p.charset = DEFAULT_CHARSET;
        },
        options: [
  ...
  component({ properties, channel, onChange }) {
    ...
    return /* @__PURE__ */ React.createElement(ConnectorForm, { properties, onChange, fields: [
      ...
      { key: "proxyPort", label: "Proxy Port", type: "number", width: "90px", disabled: (p) => !asBool(p.useProxyServer) },
      {
        ...
        refresh: true,
        onSet: (p) => {
          if (String(p.method) !== "post") p.multipart = false;
        },
        options: [
      ...
      { key: "headersVariable", label: "Map Variable", type: "text", width: "320px", placeholder: "e.g. RESTHeaders", disabled: (p) => !asBool(p.useHeadersVariable) },
      {
        ...
        onSet: (p) => {
          if (httpFormUrlEncoded(p)) {
            p.multipart = false;
            p.dataTypeBinary = false;
          }
        }
      },
      {
        ...
        onSet: (p) => {
          if (asBool(p.dataTypeBinary)) p.charset = DEFAULT_CHARSET;
        },
        options: [
          { value: true, label: "Binary" },
          { value: false, label: "Text" }
