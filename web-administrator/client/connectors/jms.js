...
function loadServerTemplates() {
  if (!templatesPromise) {
    templatesPromise = api.get("/connectors/jms/templates").then((list) => list && typeof list === "object" && !Array.isArray(list) ? list : {}).catch(() => ({}));
  }
  ...
  return {
    ...
    render: (p, ctx) => {
      ...
      function applyTemplate() {
        ...
        if (!tpl) {
          toast("That template no longer exists on the server.", "warn");
          return;
        }
      ...
      async function saveTemplate() {
        ...
        if (isPredefined(name)) {
          toast(`"${name}" is a reserved template and cannot be overwritten.`, "warn");
          return;
        }
        const body = {
    ...
    return /* @__PURE__ */ React.createElement(ConnectorForm, { properties, onChange, fields: [
      ...
      { section: "Destination Settings" },
      {
        ...
        append: (p, ctx) => checkbox("Durable", asBool(p.durableTopic), {
          ...
          onChange: (e) => {
            p.durableTopic = e.target.checked;
            ctx.onChange();
            ctx.repaint();
          }
        }).el
