...
import { ConnectorForm, portsInUseButton, listenerAddressField, postConnectorProperties, successToast, apiErrorMessage, YES_NO, defaultSourceProperties, defaultDestinationProperties, defaultListenerProperties, asBool, requireFields } from './react-forms.js';
    ...
    component({ properties, channel, onChange }) {
        ...
        async function generateEnvelope(btn, repaint) {
            ...
            try {
                ...
                const envelope = await api.post('/connectors/ws/_generateEnvelope', wsFormBody(properties, channel, Object.assign({ buildOptional: true }, opParams)), { contentType: 'application/x-www-form-urlencoded', raw: true });
                if (envelope !== null && envelope !== undefined)
                    properties.envelope = envelope;
                try {
                    const soapAction = await api.post('/connectors/ws/_getSoapAction', wsFormBody(properties, channel, opParams), { contentType: 'application/x-www-form-urlencoded', raw: true });
                    if (soapAction)
                    ...
                    ] },
                {
                    ...
                    render: (p, ctx) => {
                        ...
                        if (!ops.includes(current))
                            ops.unshift(current || WS_DEFAULT_OPERATION);
