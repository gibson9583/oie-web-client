  ...
  function paint() {
    ...
    rows.forEach((row, i) => {
      wrap.appendChild(h(
        ...
        textInput(row[0], { placeholder: "ID", disabled, class: "flex-1", onInput: (e) => {
          row[0] = e.target.value;
          commit();
        } }),
        textInput(row[1], { placeholder: "Content", disabled, class: "flex-[2]", onInput: (e) => {
          row[1] = e.target.value;
          commit();
        } }),
        textInput(row[2], { placeholder: "MIME Type", disabled, class: "flex-1", onInput: (e) => {
