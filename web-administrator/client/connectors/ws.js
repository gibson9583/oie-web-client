...
import { h, clear, textInput, select, icon, toast, taskButton } from '../core/ui.js';
import * as api from '../core/api.js';
import {
    buildForm, connectorTestButton, portsInUseButton, postConnectorProperties, successToast, apiErrorMessage,
    ...
    function paint() {
        ...
        rows.forEach((row, i) => {
            wrap.appendChild(h('div', { style: { display: 'flex', gap: '6px', marginBottom: '6px' } },
                textInput(row[0], { placeholder: 'ID', style: { flex: '1' }, onInput: (e) => { row[0] = e.target.value; commit(); } }),
                textInput(row[1], { placeholder: 'Content', style: { flex: '2' }, onInput: (e) => { row[1] = e.target.value; commit(); } }),
                textInput(row[2], { placeholder: 'MIME Type', style: { flex: '1' }, onInput: (e) => { row[2] = e.target.value; commit(); } }),
...
const wsSender = {
    ...
    render(host, { properties, channel, onChange }) {
        ...
        let services = parseDefinitionMap(properties.wsdlDefinitionMap);

        const currentPortInfo = () => {
            const ports = services.get(String(properties.service ?? ''));
        ...
        async function getOperations(btn, repaint) {
            ...
            try {
                ...
                services = parseDefinitionMap(properties.wsdlDefinitionMap);

                const firstService = services.keys().next().value ?? '';
                properties.service = firstService;
                const ports = services.get(firstService) || new Map();
        ...
        buildForm(formHost, properties, [
            { section: 'Web Service Sender Settings' },
            {
                ...
                render: (p, ctx) => {
                    const input = textInput(p.wsdlUrl ?? '', {
                        placeholder: 'http://host:port/service?wsdl', style: { flex: '1' },
            ...
            },
            {
                ...
                render: (p, ctx) => comboInput(p.service, [...services.keys()], {
            ...
            },
            {
                ...
                render: (p, ctx) => {
                    const ports = services.get(String(p.service ?? ''));
            ...
            {
                type: 'custom', label: 'Location URI', width: '420px',
                render: (p) => {
                    const info = currentPortInfo();
            ...
            {
                type: 'custom', label: 'Operation', width: '320px',
                render: (p, ctx) => {
                    const info = currentPortInfo();
                    const ops = info ? [...info.operations] : [];
