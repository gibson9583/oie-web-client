...
import {
    buildForm, connectorTestButton, pollSection, asBool, YES_NO,
...
const fileReader = {
    ...
    render(host, { properties, channel, onChange }) {
        ...
        buildForm(formHost, properties, [
            ...
            {
                key: 'errorMoveToDirectory', label: 'Error Move-to Directory', type: 'text',
                visible: (p) => p.errorReadingAction === 'MOVE' || p.errorResponseAction === 'MOVE'
            }
        ], onChange);
        host.appendChild(pollSection(properties, onChange));
        host.appendChild(h('div', { style: { marginTop: '10px' } },
            connectorTestButton({ label: 'Test Read', icon: 'folder', path: '/connectors/file/_testRead', channel, properties })));
    }
};
...
const fileWriter = {
    ...
    render(host, { properties, channel, onChange }) {
        ...
        host.appendChild(h('div', { style: { marginTop: '10px' } },
            connectorTestButton({ label: 'Test Write', icon: 'folder', path: '/connectors/file/_testWrite', channel, properties })));
    }
};
