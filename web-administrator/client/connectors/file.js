...
function fileExistsValue(p) {
    if (asBool(p.outputAppend))
        return 'append';
    if (asBool(p.errorOnExists))
        return 'error';
    return 'overwrite';
}
...
function regionPicker(p, ctx) {
    ...
    const sel = select(opts, isKnown ? current : 'Custom', {
        onChange: (e) => {
            ...
            if (v === 'Custom')
                return; // typing drives "Custom"; selecting it is a no-op (matches Swing)
            sp.region = v;
            ctx.onChange();
            if (ctx.repaint)
                ctx.repaint();
...
function hostPathField(onChange) {
    return {
        ...
        render: (p) => {
            ...
            return h('div', { class: 'flex items-center gap-[5px]' }, h('span', { class: 'text-[var(--muted,#888)] whitespace-nowrap' }, schemePrefix(p)), hostInput, h('span', '/'), pathInput);
        }
    };
}
...
function onWriterSchemeChange(properties) {
    ...
    if (properties.scheme === 'S3') {
        ...
        if (asBool(properties.outputAppend))
            properties.outputAppend = false;
    }
}
