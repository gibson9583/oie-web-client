...
function text(data: Uint8Array): string { return decoder.decode(data).replace(/\0.*$/s, ''); }

/** Read TAR members in memory; never extract paths onto the user's filesystem. */
export function* readTar(data: Uint8Array, recursive = true): Generator<{ name: string; data: Uint8Array }> {
    let nextName = '';
    ...
    for (let offset = 0; offset + 512 <= data.length;) {
        ...
        if (type === 120 || type === 103) {
            ...
            if (type === 103) globalPax = { ...globalPax, ...values };
            else localPax = values;
            continue;
        }
        if (type === 76) { nextName = text(content); continue; }
        const path = localPax.path || globalPax.path || nextName || (prefix ? `${prefix}/${name}` : name);
        localPax = {};
        nextName = '';
        if (type !== 0 && type !== 48) continue; // directories, links, devices are not message files
...
export async function* readMessageArchive(
    name: string, data: Uint8Array, recursive: boolean, assertActive: () => void = () => {}
): AsyncGenerator<MessageImportFile> {
    assertActive();
    if (/\.zip$/i.test(name)) {
        for await (const entry of readZip(data, recursive, assertActive)) yield { name: entry.name, content: decoder.decode(entry.data) };
        return;
    }
...
export function pickMessageFiles(directory: boolean): Promise<File[] | null> {
    return new Promise(resolve => {
        ...
        else input.accept = '.xml,.zip,.tar,.tar.gz,.tgz,.tar.bz2,.tbz2';
...
export async function* readMessageFiles(
    files: File[], recursive: boolean, assertActive: () => void
): AsyncGenerator<MessageImportFile> {
    for (const file of files) {
        ...
        assertActive();
        if (archive) yield* readMessageArchive(file.name, new Uint8Array(content as ArrayBuffer), recursive, assertActive);
        else yield { name: relative, content: content as string };
    }
}
