...
function collItems(inner) {
    if (inner === '' || inner === null || inner === undefined)
        return [];
...
function formatCustomMap(inner) {
    ...
    if (lists.length && strings.length === lists.length) {
        ...
        for (let i = 0; i + 1 < strings.length; i += 2)
            parts.push(`${strings[i]}=${strings[i + 1]}`);
    }
...
function formatEntryMap(node) {
    if (!node || typeof node !== 'object' || node.entry === undefined)
        return '{}';
    const parts = [];
...
export function toDisplayString(v) {
    ...
    if (keys.length === 1) {
        ...
        if (type === 'entry')
            return formatEntryMap(node);
        if (XSTREAM_SCALARS.has(type))
            return inner == null ? '' : String(inner);
        if (XSTREAM_COLLECTIONS.has(type))
            return '[' + collItems(inner).map(toDisplayString).join(', ') + ']';
        if (XSTREAM_MAPS.has(type))
            return formatEntryMap(inner);
