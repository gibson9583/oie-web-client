...
const ids = (value) => api.asList(value, 'string').map(String);
const key = (edge) => JSON.stringify([edge.dependentId, edge.dependencyId]);
const copyEdges = (edges) => edges.map(d => ({ dependentId: String(d.dependentId), dependencyId: String(d.dependencyId) }));
const pending = new WeakMap();
/** Related writes belong to the working channel, not a particular editor mount. */
export function channelDependencyState(channel) {
...
export function librarySelection(libraries, channelId) {
...
export function refreshLibraryChoices(state, libraries, channelId) {
    ...
    for (const library of state.libraries) {
        if (!current.has(library.id) && state.checked.get(library.id) !== state.initial.get(library.id))
            current.set(library.id, library);
    }
    state.libraries = [...current.values()];
}
export function dependencySelection(edges) {
...
export function refreshDependencyChoices(state, edges) {
    ...
    for (const edge of state.initial)
        if (!after.has(key(edge)))
            merged.delete(key(edge));
    for (const edge of state.all)
        if (!before.has(key(edge)))
            merged.set(key(edge), { ...edge });
    state.initial = copyEdges(edges);
    state.all = [...merged.values()];
    state.changed = hasDependencyChanges(state);
}
export function hasLibraryChanges(state) {
    return !!state && [...state.checked.keys()].some(id => state.checked.get(id) !== state.initial.get(id));
}
export function hasDependencyChanges(state) {
    if (!state)
        return false;
    const before = new Set(state.initial.map(key));
    const after = new Set(state.all.map(key));
...
export async function persistLibraryAssociations(channel, ref, version, confirmOverwrite) {
    const assertSession = captureEngineSession();
    assertSession();
    const state = ref?.current;
    if (!hasLibraryChanges(state))
        return true;
    ...
    async function attempt(override) {
        ...
        for (const id of intents) {
            if (!byId.has(id))
                throw new Error('A selected code template library was removed. Review the pending selections before saving.');
        }
        ...
        const payload = current.map(library => {
            ...
            if (intents.includes(library.id) && libraryEnabledFor(library, channel.id) !== state.checked.get(library.id)) {
                ...
                if (state.checked.get(library.id)) {
                    enabled.add(channel.id);
                    disabled.delete(channel.id);
                }
                else {
        ...
        if (needsWrite) {
            ...
            if (String(result?.overrideNeeded) === 'true') {
                if (override || !confirmOverwrite)
                    throw new Error('Code template libraries changed while saving. Your selections are retained; retry to merge them with the current libraries.');
        ...
        for (const id of intents)
            state.initial.set(id, state.checked.get(id));
        return true;
    }
...
export async function saveDependencyChanges(add, remove) {
    const assertSession = captureEngineSession();
    assertSession();
    ...
    for (const edge of add)
        merged.set(key(edge), edge);
    if (merged.size === current.length && current.every(edge => merged.has(key(edge))))
        return current;
...
export async function persistChannelDependencies(ref) {
    const assertSession = captureEngineSession();
    assertSession();
    const state = ref?.current;
    if (!hasDependencyChanges(state))
        return;
    const before = new Set(state.initial.map(key));
    const after = new Set(state.all.map(key));
    const add = state.all.filter((edge) => !before.has(key(edge)));
    const remove = state.initial.filter((edge) => !after.has(key(edge)));
