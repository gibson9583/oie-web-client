...
export function initTheme() {
    ...
    let theme = 'light';
    try {
        theme = localStorage.getItem(THEME_LAST_KEY) || 'light';
    }
    catch { /* private mode */ }
    document.documentElement.dataset.theme = theme;
    state.theme = theme;
...
/* ---- typeface ---- */
export const FONT_UI_OPTIONS = ['inter', 'plex', 'b612', 'martian', 'system'];
export const FONT_MONO_OPTIONS = ['jetbrains', 'plexmono', 'b612mono', 'martian', 'system'];
...
export function setRailCollapsed(collapsed) {
    try {
        localStorage.setItem(scopedKey('oie-rail-collapsed'), collapsed ? '1' : '0');
    }
    catch { /* private mode */ }
...
export function initRailCollapsed() {
    let collapsed = false;
    try {
        collapsed = localStorage.getItem(scopedKey('oie-rail-collapsed')) === '1';
    }
    catch { /* private mode */ }
    if (narrowViewport())
        collapsed = true; // drawer starts closed on small screens
