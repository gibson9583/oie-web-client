...
export function normalizeLineEndings(s: unknown): string {
    return String(s == null ? '' : s).replace(/\r\n/g, '\n').replace(/\r/g, '\n');
}

const span = (cls: string, text: string, tooltip?: string): HTMLSpanElement => {
...
export function detectType(text: unknown, dataType?: string | null): ContentKind {
...
function pushAttrs(out: Node[], attrStr: string): void {
    let last = 0; ATTR_RE.lastIndex = 0; let m: RegExpExecArray | null;
...
function highlightXml(xml: string, out: Node[]): void {
    let last = 0; XML_RE.lastIndex = 0; let m: RegExpExecArray | null;
...
function highlightJson(json: string, out: Node[]): void {
    const lines = json.split('\n');
    for (let i = 0; i < lines.length; i++) {
        if (i > 0) out.push(document.createTextNode('\n'));
        const line = lines[i];
        let last = 0; JSON_RE.lastIndex = 0; let m: RegExpExecArray | null;
 ...
 */
export function renderHighlighted(preEl: HTMLElement, text: unknown, { type, dataType, format = false, descriptions = null }: {
    type?: ContentKind;
