...
const subs: Record<string, Array<(value: any) => void>> = {};
...
export function subscribe(key: string, fn: (value: any) => void): () => void {
...
export function on(event: string, fn: (detail: any) => void): () => void {
...
export function setPrefScope(serverId: string | number | null | undefined, userId: string | number | null | undefined): void {
...
export const TABLE_DENSITIES: string[] = ['compact', 'normal', 'wide'];

/* On <html> so it also reaches grids rendered inside dialogs, which Radix portals
   outside the app tree. */
export function setTableDensity(density: string): void {
...
export function reapplyScopedSettings(): void {
    try {
