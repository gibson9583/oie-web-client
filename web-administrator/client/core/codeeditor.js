...
function attachCodeView(editor, opts) {
    ...
    let dragToken = null;
    const onKey = (e) => { if (e.key === 'Escape' && open) {
        e.preventDefault();
        e.stopPropagation();
        set(false);
    } };
    ...
    function buildVarsRail() {
        ...
        return h('div.ce-popout-vars', h('div.ce-popout-vars-head', 'Variables'), list, h('div.ce-popout-vars-hint', 'Click or drag to insert.'));
    }
    ...
    function openCodeView() {
        ...
        overlay = h('div.ce-popout-overlay', h('div.ce-popout-head', h('button.btn', { type: 'button', onClick: () => set(false) }, icon('chevL'), 'Back'), h('div.ce-popout-title', String(opts.popoutTitle || 'Code editor')), h('div.ce-popout-esc', 'Esc closes')), body);
    ...
    function set(next) {
        ...
        if (editor.monaco)
            editor.monaco.layout(); // automaticLayout also catches it
    }
...
let factory = (opts = {}) => {
    ...
    ensureMonaco().then((monaco) => {
        ...
        try {
            ...
            console.warn('[codeeditor] Monaco upgrade failed:', e);
        }
    });
    return editor;
};
