...
const MODES: readonly TimezoneMode[] = ['server', 'local', 'utc'];
const isMode = (v: string | null): v is TimezoneMode => !!v && (MODES as readonly string[]).includes(v);
const LOCAL_TZ = (() => {
...
export function onTimezoneChange(cb: () => void): () => void { listeners.add(cb); return () => listeners.delete(cb); }
function emit(): void { for (const cb of listeners) { try { cb(); } catch { /* listener error */ } } }

export function timezoneMode(): TimezoneMode { return mode; }
...
export function tzAbbr(zone: string = resolvedZone()): string {
...
export function resolvedAbbr(): string {
...
export function formatInZone(date: Date, zone: string = resolvedZone()): string {
...
function zoneForOffset(off: number): string {
    if (!off) return 'UTC';
...
export async function loadServerTimezone(): Promise<void> {
    if (fetched) return;
    fetched = true;
    try {
