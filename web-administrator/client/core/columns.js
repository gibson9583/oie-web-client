...
export function createColumnManager(storageKey, defaults, defaultHidden = []) {
    ...
    return {
        ...
        setHidden(key, v) { if (v)
            hidden.add(key);
        else
            hidden.delete(key); save(); },
...
export function decorateColumns(table, opts) {
    ...
    const permute = (tr) => {
        ...
        if (cells.length < pinned + presentKeys.length)
        ...
        for (const key of displayOrder) {
            ...
            if (cell)
                tr.appendChild(cell); // appendChild moves the node to the end
        }
    };
    ...
    const syncMinWidth = () => {
        let min = 0;
        for (let i = 0; i < pinned; i++)
            min += (pinnedWidths[i] || 26);
        for (const key of displayOrder)
            min += (key === lastKey ? 80 : (parseFloat(colByKey.get(key).style.width) || manager.width(key)));
    ...
    headCells.forEach((th, i) => {
        ...
        if (key === lastKey)
            return; // filler column: no resize grabber
        ...
        handle.addEventListener('dblclick', (e) => {
            ...
            for (const tr of table.querySelectorAll('tbody > tr')) {
                ...
                if (!c || c.colSpan > 1)
                    continue; // skip spanning (empty-state) cells
