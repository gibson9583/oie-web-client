...
const ce = (root: Element, tag: string): Element | null => {
    for (const n of root.childNodes) if (n.nodeType === 1 && n.nodeName === tag) return n as Element;
    return null;
};
const cbool = (root: Element, tag: string): boolean => String(ct(root, tag)).trim() === 'true';
const elChildren = (el: Element | null): Element[] => (el ? [...el.childNodes].filter((n): n is Element => n.nodeType === 1) : []);
const strList = (el: Element | null): string[] => elChildren(el).filter(n => n.nodeName === 'string').map(n => n.textContent || '');

// Map<String, String|List<String>> → [{ key, values: [] }].
function mapEntries(el: Element | null): Array<{ key: string; values: string[] }> {
...
function fmtVm(r: Element): string {
...
function fmtHttp(r: Element): string {
...
function fmtTcp(r: Element): string {
...
function fmtDatabase(r: Element): string {
...
function fmtSmtp(r: Element): string {
...
function appendUri(r: Element): string {
...
function fmtScheme(el: Element | null): string {
    if (!el) return '';
...
function fmtFile(r: Element): string {
...
function fmtDocument(r: Element): string {
    const lines: string[] = [];
...
function fmtJms(r: Element): string {
    const lines: string[] = [];
    if (cbool(r, 'useJndi')) {
...
function fmtWebService(r: Element): string {
...
function fmtDicom(r: Element): string {
...
const FORMATTERS: Record<string, (r: Element) => string> = {
...
function fmtGeneric(r: Element): string {
    const lines: string[] = [];
    let contentField: Element | null = null;
 ...
 */
export function formatSentProperties(content: unknown): string | null {
