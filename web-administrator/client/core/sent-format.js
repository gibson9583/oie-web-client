...
function mapEntries(el) {
    ...
    for (const entry of elChildren(el)) {
        ...
        let values = [];
        if (second) {
            if (second.nodeName === 'string')
                values = [second.textContent || ''];
            else {
...
function fmtVm(r) {
    ...
    for (const v of strList(ce(r, 'mapVariables')))
        lines.push(v);
...
function fmtHttp(r) {
    ...
    if (ct(r, 'username').trim())
        lines.push('USERNAME: ' + ct(r, 'username'));
    lines.push('', '[HEADERS]');
    if (cbool(r, 'useHeadersVariable'))
        lines.push(`Using variable '${ct(r, 'headersVariable')}'`);
    else
        for (const { key, values } of mapEntries(ce(r, 'headers')))
            for (const v of values)
                lines.push(`${key}: ${v}`);
    lines.push('', '[PARAMETERS]');
    if (cbool(r, 'useParametersVariable'))
        lines.push(`Using variable '${ct(r, 'parametersVariable')}'`);
    else
        for (const { key, values } of mapEntries(ce(r, 'parameters')))
            for (const v of values)
                lines.push(`${key}: ${v}`);
...
function fmtTcp(r) {
    ...
    if (cbool(r, 'overrideLocalBinding'))
        lines.push('LOCAL ADDRESS: ' + ct(r, 'localAddress') + ':' + ct(r, 'localPort'));
...
function fmtSmtp(r) {
    ...
    if (cbool(r, 'useAttachmentsVariable'))
        lines.push(`Using variable '${ct(r, 'attachmentsVariable')}'`);
    else
        for (const att of elChildren(ce(r, 'attachments')))
            lines.push(`${ct(att, 'name')} (${ct(att, 'mimeType')})`);
...
function fmtDocument(r) {
    ...
    if (!output.trim() || !/^attachment$/i.test(output))
        lines.push('URI: ' + appendUri(r));
...
function fmtJms(r) {
    const lines = [];
    if (cbool(r, 'useJndi')) {
        lines.push('PROVIDER URL: ' + ct(r, 'jndiProviderUrl'), 'INITIAL CONTEXT FACTORY: ' + ct(r, 'jndiInitialContextFactory'), 'CONNECTION FACTORY NAME: ' + ct(r, 'jndiConnectionFactoryName'), 'DESTINATION: ' + ct(r, 'destinationName'));
    }
    else {
        lines.push('CONNECTION FACTORY CLASS: ' + ct(r, 'connectionFactoryClass'));
        lines.push((cbool(r, 'topic') ? 'TOPIC: ' : 'QUEUE: ') + ct(r, 'destinationName'));
    }
    if (ct(r, 'clientId').trim())
        lines.push('CLIENT ID: ' + ct(r, 'clientId'));
    if (ct(r, 'username').trim())
        lines.push('USERNAME: ' + ct(r, 'username'));
    const conn = mapEntries(ce(r, 'connectionProperties'));
    if (conn.length) {
        lines.push('', '[CONNECTION PROPERTIES]');
        for (const { key, values } of conn)
            lines.push(`${key}: ${values[0] ?? ''}`);
    }
    lines.push('[CONTENT]', ct(r, 'template'));
    return lines.join('\n');
...
function fmtWebService(r) {
    ...
    if (ct(r, 'username').trim())
        lines.push('USERNAME: ' + ct(r, 'username'));
    if (ct(r, 'service').trim())
        lines.push('SERVICE: ' + ct(r, 'service'));
    if (ct(r, 'port').trim())
        lines.push('PORT / ENDPOINT: ' + ct(r, 'port'));
    if (ct(r, 'locationURI').trim())
        lines.push('LOCATION URI: ' + ct(r, 'locationURI'));
    if (ct(r, 'soapAction').trim())
        lines.push('SOAP ACTION: ' + ct(r, 'soapAction'));
    if (cbool(r, 'useHeadersVariable'))
        lines.push('', '[HEADERS]', `Using variable '${ct(r, 'headersVariable')}'`);
    else {
...
function fmtDicom(r) {
    ...
    if (ct(r, 'localHost').trim())
        lines.push('LOCAL ADDRESS: ' + ct(r, 'localHost') + ':' + ct(r, 'localPort'));
    if (ct(r, 'applicationEntity').trim())
        lines.push('REMOTE APPLICATION ENTITY: ' + ct(r, 'applicationEntity'));
    if (ct(r, 'localApplicationEntity').trim())
        lines.push('LOCAL APPLICATION ENTITY: ' + ct(r, 'localApplicationEntity'));
    if (ct(r, 'username').trim())
        lines.push('USERNAME: ' + ct(r, 'username'));
    lines.push('', '[CONTENT]', ct(r, 'template'));
    return lines.join('\n');
...
function fmtGeneric(r) {
    ...
    for (const n of elChildren(r)) {
        ...
        if (tag.startsWith('@') || SKIP.has(tag))
            continue;
        if (CONTENT_FIELDS.includes(tag) && !contentField) {
            contentField = n;
            continue;
        }
        if (elChildren(n).length)
            continue; // skip nested structures in the summary
        const val = (n.textContent || '').trim();
        if (val)
            lines.push(`${tag.replace(/([A-Z])/g, ' $1').toUpperCase().trim()}: ${val}`);
    }
    if (contentField)
        lines.push('', '[CONTENT]', contentField.textContent || '');
    return lines.join('\n');
}
/**
...
export function formatSentProperties(content) {
    ...
    if (!/^com\.mirth\.connect\.connectors\..*Properties$/.test(root.nodeName))
        return null;
    const fmt = FORMATTERS[root.nodeName];
    try {
        return fmt ? fmt(root) : fmtGeneric(root);
    }
    catch {
        return null;
    }
