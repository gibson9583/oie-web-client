...
export function clear(el) { while (el.firstChild)
...
export function fmtDate(value) {
    ...
    if (typeof value === 'object')
        millis = value.time ?? value.timestamp ?? null; // Calendar JSON
    if (millis === null)
        return '';
...
export function detailModal({ title, badge, meta, sections = [], copy } = {}) {
    ...
    const badgeEl = badge
        ? h('span.tag', { class: 'font-[650]', style: { color: tone, borderColor: tone } }, String(badge.text).toUpperCase())
        : null;
    const copyText = copy != null ? copy : sections.map(s => s.text).join('\n\n');
    return modal({
        title,
        size: 'wide',
        body: h('div', { class: 'flex flex-col gap-2 min-w-[558px] max-w-[80vw]' }, (badge || meta) ? h('div', { class: 'flex gap-[13px] items-center flex-wrap' }, badgeEl, meta ? h('span.mono.text-text-faint', String(meta)) : null) : null, ...sections.flatMap(s => [
...
function visibleMenuItems(items, group) {
    ...
    for (const item of items) {
        ...
        if (item.task && !checkTask(item.group || group, item.task))
            continue;
        if (item.items) {
...
function domContextMenu(x, y, items) {
    ...
    const render = (item, nested) => {
        ...
        if (item.header) {
            menu.appendChild(h('div.ctx-head', { role: 'presentation' }, h('div.ctx-head-name', item.label), item.sub ? h('div.ctx-head-sub', item.sub) : null));
            return;
        }
        ...
        if (item.items) {
            menu.appendChild(h('div.ctx-head', { role: 'presentation' }, h('div.ctx-head-name', item.label)));
...
function menuKeys(e) {
    ...
    if (e.key.length === 1 && /\S/.test(e.key) && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if ((it.textContent || '').trim().toLowerCase().startsWith(key)) {
                e.preventDefault();
                it.focus();
                return;
            }
...
export function closeContextMenu({ restore = true } = {}) {
    if (openRendered) {
        const handle = openRendered;
        openRendered = null;
        handle.close({ restore });
        return;
    }
    if (!openMenu)
        return;
...
export function tabs(defs, { onChange, active = 0, label = 'Tabs' } = {}) {
    ...
    let current = -1;
    const buttons = defs.map((def, i) => h('button.tab', { role: 'tab', tabindex: '-1', onClick: () => select(i) }, def.label));
    buttons.forEach(b => bar.appendChild(b));
    bar.addEventListener('keydown', (e) => {
        let to = -1;
        if (e.key === 'ArrowRight')
            to = (current + 1) % buttons.length;
        else if (e.key === 'ArrowLeft')
            to = (current - 1 + buttons.length) % buttons.length;
        else if (e.key === 'Home')
            to = 0;
        else if (e.key === 'End')
            to = buttons.length - 1;
        if (to < 0 || !buttons.length)
            return;
        e.preventDefault();
        select(to);
        buttons[to].focus();
...
export class DataTable {
    ...
    constructor(columns, options = {}) {
        ...
        this.manager = options.columnsKey
            ? createColumnManager(options.columnsKey, Object.fromEntries(columns.filter(c => c.width).map(c => [c.key, parseInt(c.width, 10) || 120])), [...this.defaultHidden])
            : null;
    ...
    visibleColumns() {
        if (this.manager)
            return this.columns.filter(c => !this.manager.isHidden(c.key));
    ...
    openColumnsMenu(e) {
        ...
        const items = this.columns.map(col => ({
            ...
            onClick: () => {
                if (this.hidden.has(col.key))
                    this.hidden.delete(col.key);
                else if (this.columns.length - this.hidden.size > 1)
                    this.hidden.add(col.key);
                this.saveHidden();
                this.render();
            }
        }));
    ...
    render() {
        ...
        if (options.columnsMenu && !this.manager)
            headRow.addEventListener('contextmenu', (e) => this.openColumnsMenu(e));
...
export function initTruncationTitles() {
    if (truncationTitlesInstalled)
        return;
    truncationTitlesInstalled = true;
    document.addEventListener('mouseover', (e) => {
        const target = e.target instanceof Element ? e.target : null;
        const cell = target?.closest('table.dt td, table.dt th');
        if (!cell)
            return;
        if (cell.title && cell.dataset.autoTitle === undefined)
            return; // renderer's own title
        ...
        let truncated = false;
        for (let el = target; el; el = el === cell ? null : el.parentElement) {
...
export function taskButton(label, iconName, onClick, opts = {}) {
    ...
    if (opts.task && !checkTask(opts.group, opts.task))
        return null;
    return h(`button.btn${opts.primary ? '.btn-primary' : ''}${opts.danger ? '.btn-danger' : ''}`, { onClick, disabled: opts.disabled, title: opts.title || null }, iconName ? icon(iconName) : null, label);
}
