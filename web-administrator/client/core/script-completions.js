...
function libraryInScope(lib, channelId) {
    if (idSet(lib.enabledChannelIds).includes(channelId))
        return true;
...
function normalizeNamespaceLiterals(code) {
    ...
    return String(code).replace(/((?:var\s+)?[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\s*=\s*\{((?:[\s,]*[A-Za-z_$][\w$]*\s*:\s*\{\})+[\s,]*)\}/g, (whole, lhs, body) => {
