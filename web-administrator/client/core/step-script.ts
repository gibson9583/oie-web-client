...
function convertIdentifier(id: unknown): string {
...
function replacementPairs(el: any): Array<[any, any]> {
...
function conditionClause(field: any, condition: any, values: any, existsStyle: 'xml' | 'length'): string {
...
function messageBuilderScript(el: any): string {
...
function xsltScript(el: any): string {
...
function externalScript(el: any): string {
    // loadFiles = false (design-time preview)
    return `// External script will be loaded on deploy\n// Path: ${el.scriptPath ?? ''}\n`;
}

function destinationSetFilterScript(el: any): string {
...
function ruleBuilderScript(el: any): string {
...
const GENERATORS: Record<string, (el: any) => string> = {
 ...
 */
export function generateElementScript(element: any, getChildren?: (el: any) => any[]): string | null {
...
function iteratorScript(element: any, getChildren?: (el: any) => any[]): string {
    ...
    const enabled = kids.filter((c: any) => c && c.enabled !== false);

    const pre = enabled.map((c: any) => iterPre(c)).filter(Boolean).map((s: any) => s + '\n').join('');
    const body = enabled.map((c: any) => '\n' + (iterIteration(c, getChildren) ?? generateElementScript(c, getChildren) ?? '') + '\n').join('');
    const post = enabled.map((c: any) => iterPost(c)).filter(Boolean).map((s: any) => s + '\n').join('');
...
function iterPre(el: any): string | null {
...
function iterIteration(el: any, getChildren?: (el: any) => any[]): string | null {
    if (el.__type === 'com.mirth.connect.plugins.mapper.MapperStep') {
...
function iterPost(el: any): string | null {
    if (el.__type === 'com.mirth.connect.plugins.mapper.MapperStep') {
        const scopeMap = SCOPE_MAP[el.scope] || SCOPE_MAP.CHANNEL;
