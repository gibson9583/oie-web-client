...
const isMode = (v) => !!v && MODES.includes(v);
const LOCAL_TZ = (() => {
    try {
...
function emit() { for (const cb of listeners) {
    try {
        cb();
    }
...
export function resolvedAbbr() {
    if (mode === 'server')
        return serverLabel || tzAbbr(serverZone || 'UTC');
...
export function formatInZone(date, zone = resolvedZone()) {
    let parts;
    try {
        parts = formatterFor(zone).formatToParts(date);
    }
    catch {
    ...
    let hh = get('hour');
    if (hh === '24')
        hh = '00'; // Intl hour12:false can emit "24" at midnight
...
export async function loadServerTimezone() {
    ...
    try {
        ...
        if (!m)
            return; // unknown format — keep UTC fallback
