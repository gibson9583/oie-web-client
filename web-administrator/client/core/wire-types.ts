/*
 * Engine model + wire-shape types for the @oie/web-api surface.
 *
 * Model types are GENERATED from the engine's OpenAPI spec (./oie-schema.d.ts,
 * produced by packages/web-api/gen-schema.mjs from /api/openapi.json) and
 * re-exported below, so they track the engine's Java model automatically.
 *
 ...
 *
 * Where that caveat used to be prose you had to remember, it is in the types:
 ...
 * `channel.destinationConnectors` as an array does not compile, and the
 * compiler points you at `destinationsOf()` / `elementsToArray()` instead.
 * The overrides run through the whole channel tree: `WireChannel` ->
 * `WireConnector` -> `WireTransformer`/`WireFilter`, so a class-keyed element map
 * is never mistaken for a `Step[]`/`Rule[]` at any depth.
 *
 * This module is types-only: its generated .js is empty and never imported —
 * every import of it is `import type`, erased at transpile time.
 */
