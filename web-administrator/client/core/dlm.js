...
export function dlmLooksLikeMessageId(text) {
 ...
 */
export function dlmSuggestScopeIds(text, metaColumns = []) {
    const t = dlmNormalize(text);
    if (!t)
        return [];
    if (dlmLooksLikeMessageId(t))
        return ['message_id'];
    const out = ['raw', 'source_map'];
 ...
 */
export function dlmBuildDecision(raw, opts) {
    ...
    if (scopes.includes('legacy_text') && scopes.length === 1) {
        ...
        if (opts.textSearchRegex)
            params.textSearchRegex = true;
    ...
    for (const id of activeScopes) {
        ...
        if (!scope || scope.kind !== 'content' || !scope.param)
            continue;
