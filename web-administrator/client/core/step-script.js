...
function replacementPairs(el) {
    ...
    if (!r)
        return [];
    if (r.entry)
        r = r.entry; // XStream map/list wrapper
...
function destinationSetFilterScript(el) {
    ...
    let call;
    if (el.behavior === 'REMOVE')
        call = `remove([${ids}])`;
    else if (el.behavior === 'REMOVE_ALL_EXCEPT')
        call = `removeAllExcept([${ids}])`;
    else
        call = 'removeAll()';
    return `if (${clause}{\n\tdestinationSet.${call};\n}\n`;
}
...
export function generateElementScript(element, getChildren) {
    ...
    if (ITERATOR_TYPES.has(type))
        return iteratorScript(element, getChildren);
...
function iterIteration(el, getChildren) {
    ...
    if (ITERATOR_TYPES.has(el.__type))
        return iteratorScript(el, getChildren);
