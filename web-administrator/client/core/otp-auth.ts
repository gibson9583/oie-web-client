...
import { registerLoginAuthenticator, type LoginAuthContext } from './login-auth.js';
...
function qrImage(text: string, size: number = 148): HTMLElement | null {
    try {
        const qr = qrcode(0, 'M');
        qr.addData(text);
        qr.make();
...
function groupSecret(secret: string | null | undefined): string {
...
function promptCode({ title, intro, extra, confirmLabel }: {
    title: string; intro: string; extra?: HTMLElement | null; confirmLabel?: string;
}): Promise<string | null> {
    const { h, modal } = ui();
...
function enrollSetup(info: { secret?: string; otpauthUri?: string }): HTMLElement {
    const { h } = ui();
...
async function authenticate(ctx: LoginAuthContext): Promise<any> {
    let info: any;
    try {
...
export function registerBuiltinOtpAuthenticator(): void {
