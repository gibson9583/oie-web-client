...
export function normalizeLayout(raw) {
    ...
    for (const [id, v] of Object.entries(src)) {
        if (!v || typeof v !== 'object')
            continue;
        const out = {};
        if (typeof v.group === 'string' && v.group)
            out.group = v.group;
...
export function mergeNav(items, layout, { sectionOrder = [], sectionRank = {}, legacySections = {} } = {}) {
    ...
    const sectionOf = (it) => (it.section && legacySections[it.section]) || it.section || 'Plugins';
    ...
    for (const g of groups)
        g.items.sort((a, b) => a.order - b.order || String(a.label).localeCompare(String(b.label)));
    return groups;
}
...
function dropEmpty(l) {
    for (const id of Object.keys(l.items)) {
        if (!Object.keys(l.items[id]).length)
            delete l.items[id];
    }
