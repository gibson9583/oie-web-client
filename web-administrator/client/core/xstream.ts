...
const asList = (v: unknown): unknown[] => (v === null || v === undefined || v === '') ? [] : (Array.isArray(v) ? v : [v]);
...
function collItems(inner: unknown): unknown[] {
    if (inner === '' || inner === null || inner === undefined) return [];
    if (Array.isArray(inner)) return inner;
    if (typeof inner === 'object') {
        const node = inner as XNode;
...
function formatCustomMap(inner: unknown): string | null {
...
function formatEntryMap(node: unknown): string {
    if (!node || typeof node !== 'object' || (node as XNode).entry === undefined) return '{}';
    const parts: string[] = [];
    for (const entry of asList((node as XNode).entry)) {
        if (!entry || typeof entry !== 'object') continue;
        const pair = parseMapEntry(entry as XNode);
...
export function mapNode(mc: unknown): unknown {
    let cur: unknown = (mc && typeof mc === 'object' && (mc as XNode).content !== undefined) ? (mc as XNode).content : mc;
    for (let i = 0; i < 5 && cur && typeof cur === 'object' && (cur as XNode).entry === undefined; i++) {
        const keys = Object.keys(cur).filter(k => !k.startsWith('@'));
        if (keys.length !== 1) break;
        cur = (cur as XNode)[keys[0]];
    }
    return cur;
}

// One entry node -> [key, value] (value rendered Java-toString style).
function parseMapEntry(entry: XNode): [string, string] | null {
...
export function mappingEntries(mc: unknown): Array<[string, string]> {
