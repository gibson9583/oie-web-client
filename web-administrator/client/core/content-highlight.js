...
export function detectType(text, dataType) {
    ...
    if (dt.includes('HL7V2') || dt === 'HL7')
        return 'hl7v2';
    if (dt === 'XML' || dt.includes('HL7V3'))
        return 'xml';
    if (dt === 'JSON')
        return 'json';
...
function pushAttrs(out, attrStr) {
    ...
    while ((m = ATTR_RE.exec(attrStr)) !== null) {
        if (m.index > last)
            out.push(document.createTextNode(attrStr.slice(last, m.index)));
    ...
    if (last < attrStr.length)
        out.push(document.createTextNode(attrStr.slice(last)));
}
function highlightXml(xml, out) {
    ...
    while ((m = XML_RE.exec(xml)) !== null) {
        if (m.index > last)
            out.push(document.createTextNode(xml.slice(last, m.index)));
        const [full, comment, cdata, bracket, tagName, attrs, close, text] = m;
        if (comment)
            out.push(span('tok-xml-comment', comment));
        else if (cdata)
            out.push(span('tok-xml-bracket', cdata));
            ...
            if (attrs)
                pushAttrs(out, attrs);
            if (close)
                out.push(span('tok-xml-bracket', close));
        }
        else if (text)
            out.push(document.createTextNode(text));
        else
            out.push(document.createTextNode(full));
        last = m.index + full.length;
    }
    if (last < xml.length)
        out.push(document.createTextNode(xml.slice(last)));
}
/* ---- JSON highlighter ------------------------------------------------------- */
...
function highlightJson(json, out) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = JSON_RE.exec(line)) !== null) {
            if (m.index > last)
                out.push(document.createTextNode(line.slice(last, m.index)));
            const [full, key, str, num, bool, bracket] = m;
            if (key) {
                out.push(span('tok-json-key', key));
                out.push(document.createTextNode(':'));
            }
            else if (str)
                out.push(span('tok-json-string', str));
            else if (num)
                out.push(span('tok-json-number', num));
            else if (bool)
                out.push(span('tok-json-bool', bool));
            else if (bracket)
                out.push(span('tok-json-bracket', bracket));
            else
                out.push(document.createTextNode(full));
            last = m.index + full.length;
        }
        if (last < line.length)
            out.push(document.createTextNode(line.slice(last)));
    }
}
...
function hl7Name(seg, field, comp, descriptions) {
    if (descriptions) {
        if (comp > 1 && descriptions[`${seg}.${field}.${comp}`])
            return descriptions[`${seg}.${field}.${comp}`];
        if (descriptions[`${seg}.${field}.1`] && comp <= 1)
            return descriptions[`${seg}.${field}.1`];
        if (descriptions[`${seg}.${field}`])
            return descriptions[`${seg}.${field}`];
    }
    return undefined;
}
...
function highlightField(value, seg, field, enc, descriptions, out) {
    ...
    for (let r = 0; r < reps.length; r++) {
        if (r > 0)
            out.push(span('tok-hl7-sep', enc.repetitionSep));
        const comps = reps[r].split(enc.componentSep);
        for (let c = 0; c < comps.length; c++) {
            if (c > 0)
                out.push(span('tok-hl7-sep', enc.componentSep));
            if (!comps[c])
                continue;
            const subs = comps[c].split(enc.subComponentSep);
            for (let sc = 0; sc < subs.length; sc++) {
                if (sc > 0)
                    out.push(span('tok-hl7-sep', enc.subComponentSep));
                if (!subs[sc])
                    continue;
...
function highlightHl7(hl7, descriptions, out) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('MSH'))
            highlightMshLine(line, enc, descriptions, out);
        else if (/^[A-Z][A-Z\d]{2}/.test(line))
            highlightSegmentLine(line, enc, descriptions, out);
        else
...
export function renderHighlighted(preEl, text, { type, dataType, format = false, descriptions = null } = {}) {
    ...
    if (format && kind === 'xml')
        body = prettyPrintXml(body);
    else if (format && kind === 'json')
        body = prettyPrintJson(body);
    const out = [];
    if (kind === 'xml')
        highlightXml(body, out);
    else if (kind === 'json')
        highlightJson(body, out);
    else if (kind === 'hl7v2')
        highlightHl7(body, descriptions, out);
    else
        out.push(document.createTextNode(body));
