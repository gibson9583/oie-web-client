...
export async function saveChannelModel(channel, options) {
    ...
    const sameUser = (model) => options.userId != null && model?.exportData?.metadata?.userId != null
    ...
    try {
        ...
        if (!state.isNew) {
            ...
            if (conflict && !sameUser(current) && !await confirm(options.confirmConflict()))
                return false;
        ...
        if (state.isNew) {
            ...
            if (String(accepted) === 'false') {
                ...
                assertSession();
                if (!latest || latest.id !== channel.id)
                    throw new Error('The channel was removed. Reopen the channel list before saving.');
                if (!sameUser(latest) && !await confirm(options.confirmConflict()))
                    return false;
