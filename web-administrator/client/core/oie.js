...
export function messageStatusTag(status) {
    switch (status) {
        case 'SENT':
        case 'TRANSFORMED': return 'accent';
...
export function destinationsOf(channel) {
    if (!channel || !channel.destinationConnectors)
        return [];
...
function transformersOf(channel) {
    ...
    for (const d of destinationsOf(ch)) {
        if (d && d.transformer)
            out.push(d.transformer);
        if (d && d.responseTransformer)
            out.push(d.responseTransformer);
    }
    return out;
}
