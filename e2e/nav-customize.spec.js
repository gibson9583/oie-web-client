...
const rail = (page) => page.locator('.rail-nav');
const gear = (page) => page.locator('#rail-customize');
const layoutPref = (page) => page.evaluate(() => {
...
async function customize(page, on) {
...
test('RBAC still wins over the preference', async ({ page }) => {
    ...
    const ids = await rail(page).locator('[data-nav-item]').evaluateAll((els) => els.map((e) => e.dataset.navItem));
