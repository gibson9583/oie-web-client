...
test('Import Channel accepts a <list> file and imports every channel in it', async ({ page }) => {
    ...
    await chooser.setFiles({
        ...
        // The shape a real engine emits: a BARE <list>, with the version stamped
        // on each serialized <channel>. (Verified against 4.6.0 — XStream stamps
        // the object, not the wrapper.)
  ...
  <channel version="4.5.0"><id>c-bulk-2</id><name>Bulk Two</name><revision>1</revision></channel>
    ...
    // Same version as the server, so nothing should ask to convert anything.
    // Reading the version off the <list> root instead of the children made this
    // prompt on every list import, including one this engine had just written.
    await expect(page.getByRole('dialog', { name: 'Select an Option' })).toBeHidden();
...
test('a list whose channels predate the server asks once, not once per channel', async ({ page }) => {
    /* The migration gate is per FILE (Swing prompts once per document), but the
       versions it judges are the children's. Two old channels in one list =>
    ...
    await (await chooserPromise).setFiles({
        name: 'old-channels.xml',
        mimeType: 'application/xml',
        buffer: Buffer.from(`<list>
  <channel version="3.9.0"><id>c-old-1</id><name>Old One</name><revision>1</revision></channel>
  <channel version="3.9.0"><id>c-old-2</id><name>Old Two</name><revision>1</revision></channel>
</list>`)
    });

    const prompt = page.getByRole('dialog', { name: 'Select an Option' });
    await expect(prompt).toBeVisible();
    await expect(prompt).toContainText('originated from Open Integration Engine version 3.9.0');
    await prompt.getByRole('button', { name: 'Yes', exact: true }).click();

    await expect.poll(() => posted.length, { timeout: 8000 }).toBe(2);
    // One prompt for the file, not one per channel.
    await expect(prompt).toBeHidden();
});

test('a group export missing a referenced channel fails instead of writing an incomplete backup', async ({ page }) => {
    /* The hydration answer "forgets" c-stopped (concurrent deletion, RBAC
       filtering, or a partial engine response): the export must abort loudly,
       not produce a valid-looking ZIP silently missing Group B's channel. */
    const PARTIAL_XML = `<list version="4.5.0">
      <channel version="4.5.0"><id>c-started</id><name>Demo Started</name><revision>1</revision></channel>
    </list>`;
    await mockEngine(page, {
        'GET /channels': xmlOr(PARTIAL_XML, { list: { channel: [
            { '@version': '4.5.0', id: 'c-started', name: 'Demo Started', revision: 1 },
            { '@version': '4.5.0', id: 'c-stopped', name: 'Demo/Stopped', revision: 1 }
        ] } }),
        'GET /channelgroups': xmlOr(GROUPS_XML, GROUPS_JSON)
...
test('Export All Channels aborts when the engine omits a channel this view shows', async ({ page }) => {
    const ONLY_ONE = `<list version="4.5.0">
      <channel version="4.5.0"><id>c-started</id><name>Demo Started</name><revision>1</revision></channel>
    </list>`;
    await mockEngine(page, {
        'GET /channels': xmlOr(ONLY_ONE, { list: { channel: [
            { '@version': '4.5.0', id: 'c-started', name: 'Demo Started', revision: 1 },
            { '@version': '4.5.0', id: 'c-stopped', name: 'Demo/Stopped', revision: 1 }
        ] } })
    });
