...
test('column menu hides, persists, and restores a column', async ({ page }) => {
    ...
    await page.getByRole('menu').getByRole('menuitem', { name: 'Description' }).click();
    ...
    await openColumnMenu();
    await page.getByRole('menu').getByRole('menuitem', { name: 'Restore Default' }).click();
    await expect(description()).toBeVisible();

    // Name is pinned — it must never appear as a hideable toggle.
    await openColumnMenu();
    await expect(page.getByRole('menu').getByRole('menuitem', { name: 'Name', exact: true })).toHaveCount(0);
});
...
test('right-clicking the empty channel list opens a context menu with New Channel', async ({ page }) => {
    ...
    await expect(page.getByRole('menu').getByRole('menuitem', { name: 'New Channel', exact: true })).toBeVisible();
});
